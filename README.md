...
| `ENABLE_CDP_STEALTH_PATCH` | `false` | **Validated broken — do not enable.** Turns on the [rebrowser-patches](https://github.com/rebrowser/rebrowser-patches) CDP fix baked into the image at build time (stops the Playwright driver from calling `Runtime.enable`, which is what closes the `hasCDP` leak Iphey flags). The patch installs cleanly (required downgrading Playwright to `1.52.0`, the newest version whose driver still ships the unbundled layout it can target), but live-testing it found it breaks execution-context resolution: `page.evaluate` errors on Iphey (`Cannot read properties of undefined (reading 'evaluateExpression')`), and it *regresses* `test_rebrowser_bot_detector_finds_no_leak` (a `pwInitScripts` leak that doesn't occur with the patch off) — it conflicts with this service's heavy use of `add_init_script` for stealth. Left in place off-by-default rather than reverted, since the infra (Playwright pin, Dockerfile build step) is inert when off and ripping it out would just mean re-deriving all of this on the next attempt. See `tests/test_live_fingerprint.py::test_iphey_trustworthy` for the full history, including an earlier independent attempt (`rebrowser-playwright` standalone package) that failed the same way via a deadlock. |
