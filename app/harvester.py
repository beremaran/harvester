"""Stealth browser capture and scraper contract."""
...
def _build_scraper_headers(request_headers: dict[str, Any], cookie_header: str) -> dict[str, str]:
...
class Harvester:
    ...
    async def _capture_request_headers(self, page: Page, state: dict[str, Any]) -> None:
        async def handle(route: Any) -> None:
            ...
            try:
                ...
                if main_frame_navigation:
                    state["latest_request_headers"] = dict(await request.all_headers())
            except Exception:  # noqa: BLE001 - request metadata is best effort
                pass
    ...
    async def harvest(self, req: HarvestRequest) -> HarvestResponse:
        ...
        try:
            async with self.open_stealth_page(req) as (context, page):
                await self._capture_request_headers(page, state)
                ...
                cookie_header = ""
                if resp.final_url and urlsplit(resp.final_url).scheme in {"http", "https"}:
                ...
                resp.request_headers = {
                    ...
                    for name, value in state["latest_request_headers"].items()
                }
                resp.response_headers = {
                    str(name).lower(): str(value) for name, value in raw_response_headers.items()
                }
                resp.scraper_headers = _build_scraper_headers(
                    state["latest_request_headers"], cookie_header
                )
